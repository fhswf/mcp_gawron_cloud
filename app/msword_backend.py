...
class MsWordDocumentBackend(DeclarativeDocumentBackend):
    ...
    def convert(self) -> DoclingDocument:
        ...
        if self.is_valid():
            ...
            doc = self._walk_linear(
                self.docx_obj.element.body, self.docx_obj, doc)
            return doc
        else:
            raise RuntimeError(
        ...
        for element in body:
            ...
            # Check for the sdt containers, like table of contents
                ...
                sdt_content = element.find(
                    ".//w:sdtContent", namespaces=namespaces)
                if sdt_content is not None:
                    # Iterate paragraphs, runs, or text inside <w:sdtContent>.
                    paragraphs = sdt_content.findall(
                        ".//w:p", namespaces=namespaces)
                    for p in paragraphs:
    ...
    def _get_numId_and_ilvl(
        ...
        if numPr is not None:
            ...
            numId_elem = numPr.find(
                "w:numId", namespaces=paragraph._element.nsmap)
            ilvl_elem = numPr.find(
                "w:ilvl", namespaces=paragraph._element.nsmap)
            numId = numId_elem.get(
                self.XML_KEY) if numId_elem is not None else None
            ilvl = ilvl_elem.get(
                self.XML_KEY) if ilvl_elem is not None else None
    ...
    def _handle_textbox_content(
        ...
        textbox_group = doc.add_group(
            label=GroupLabel.SECTION, parent=self.parents[level -
                                                          1], name="textbox"
        )
        ...
        container_paragraphs = self._collect_textbox_paragraphs(
            textbox_elements)
    ...
    def _handle_equations_in_text(self, element, text):
        ...
        for i_substr, substr in enumerate(texts_and_equations):
            ...
            if substr in output_text[init_i:]:
                ...
                if i_substr > 0:
                    output_text = output_text[:init_i] + \
                        substr + output_text[init_i:]
                    init_i += len(substr)
                else:
        ...
        if p_style_id in ["Title"]:
            ...
            if (paragraph.text is None or len(paragraph.text.strip()) == 0) and len(
                ...
                for eq in equations:
                    ...
                    text_tmp = "" if len(
                        split_text_tmp) == 1 else split_text_tmp[1]
    ...
    def _handle_tables(
        ...
        for row_idx, row in enumerate(table.rows):
            _log.debug(
                f"Row index {row_idx} with {len(row.cells)} populated cells")
            col_idx = 0
